...
// Seed a starter partner (Galattica) if none exist yet.
...
if (partnerCount === 0) {
  ...
  ).run('Galattica', '', 'https://galattica.co.za', 0);
  console.log('Seeded 1 partner (Galattica).');
}

// Inserts a few starter blog posts if the table is empty.
// Run manually with:  npm run seed
const count = db.prepare(`SELECT COUNT(*) AS c FROM blogs`).get().c;

if (count > 0) {
  console.log(`Blogs table already has ${count} post(s); skipping blog seed.`);
  process.exit(0);
}
...
Want a tailored assessment? [Get in touch](/#contact) with the Yanova Solutions team.`,
...
Vous souhaitez un audit personnalisé ? [Contactez](/#contact) l'équipe Yanova Solutions.`,
...
const insert = db.prepare(
  `INSERT INTO blogs
     (slug, title_en, title_fr, excerpt_en, excerpt_fr, content_en, content_fr, cover_image, tags, published)
   VALUES (@slug, @title_en, @title_fr, @excerpt_en, @excerpt_fr, @content_en, @content_fr, @cover_image, @tags, 1)`
);

for (const p of posts) {
  insert.run({ ...p, slug: slugify(p.title_en) });
}
